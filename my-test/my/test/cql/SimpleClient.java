...
public class SimpleClient {
	...
	public void connect(String node) {
		...
		builder.socketOptions().setReadTimeoutMillis(60 * 60 * 1000);
		builder.socketOptions().setConnectTimeoutMillis(60 * 60 * 1000);
	...
	public void testCounterColumn() {
		...
		System.out
				.println(String
						.format("%-30s\t%-20s\t%-20s\n%s", "url_name",
								"page_name", "counter_value counter",
								"-------------------------------+-----------------------+--------------------"));
		for (Row row : results) {
			System.out.println(String.format("%-30s\t%-20s\t%-20s",
		...
		for (Row row : results) {
			for (int i = 0; i < size; i++) {
				...
				case LIST:
					...
					break;
				case MAP:
					System.out.print(row.getMap(i, java.util.Date.class,
							String.class));
					break;
				default:
					System.out.print(row.getString(i));
	...
	public void testInsert() {
		session.execute("INSERT INTO simplex.songs (id, title, album, artist, tags) "
				...
				+ "'Joséphine Baker',"
				+ "{'jazz', '2013'})"
				+ " USING TTL 86400" + " and TIMESTAMP 1318452291034" + ";");
	}
